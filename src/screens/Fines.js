...
const ComplaintCardText = ({ label, value, isReadMore = false, rs = null }) => (
	<View style={{ flexDirection: 'row', paddingVertical: 5 }}>
		...
		<View style={{ flex: 7, flexDirection: 'row', justifyContent: 'space-between' }}>
			...
			{rs ? <TextComponent type={FontType.BOLD} style={{ color: Colors.green }}>{`Rs: ${rs}`}</TextComponent> : null}
		</View>
	</View>
);
		...
		<ScrollView
			...
			{List.map((data, i) => (
				<View
					...
					/>
					<ComplaintCardText label="Vehicle No" value={`${data.vehicleNumber}`} rs={data.fineAmount} />
