...
import { StyleSheet, View, ScrollView, TouchableNativeFeedback, FlatList, TouchableOpacity, BackHandler, ActivityIndicator } from 'react-native';
...
export default class ChargeFine extends Component {
	...
	handleChange = (fineData, Fines) => {
		...
		let IsSelected = SelectedFines.filter((selectedfines) => selectedfines.fineId === fineData.fineId);
		if (IsSelected.length > 0) {
			selectedfines = SelectedFines.filter((selectedfines) => selectedfines.fineId !== fineData.fineId);
		} else {
	...
	checkForSelected = (fineId) => Object.keys(this.state.SelectedFines.find((fine) => fine.fineId === fineId) || false).length;
		...
		return (
			<View
				...
				<TouchableOpacity
					...
					<IconComponent type={IconType.AntDesign} name={SearchText ? 'closecircleo' : 'search1'} size={20} />
				</TouchableOpacity>
			</View>
		);
	};
		...
		new Services()
			...
			.then((res) => {
				...
				if (paymentType === 'PAY_NOW') {
					this.props.navigation.navigate('PaymentSuccess', JSON.stringify(data));
				} else {
	...
	render() {
		...
		const TotalAmount = SelectedFines.reduce((sum, { fineCost }) => sum + fineCost, 0);
		const { vehicleComplaintId } = this.props.route.params;
		return (
			<View style={{ flex: 1, backgroundColor: Colors.white }}>
				<SCLAlert
					...
					headerIconComponent={<IconComponent type={IconType.Ionicons} name="ios-thumbs-up" size={32} color="white" />}>
					<SCLAlertButton onPress={() => this.submitFine('PAY_NOW')} theme="success">
						Pay Now
					</SCLAlertButton>
					<SCLAlertButton onPress={() => this.submitFine('PAY_LATER')} theme="default">
						Pay Later
					</SCLAlertButton>
				</SCLAlert>
              ...
              <View
				...
				{Loading ? (
					...
					<FlatList
						data={Fines}
						renderItem={({ item }) => (
							<TouchableNativeFeedback onPress={() => this.handleChange(item, Fines)}>
								<View
									style={{
										width: widthPerc(100),
										paddingVertical: 10,
									...
									<View style={{ flex: 7, paddingRight: 10 }}>
										<TextComponent style={styles.label}>{item.fineType}</TextComponent>
									</View>
									<View style={{ flex: 2 }}>
										<TextComponent style={[styles.label]} type={FontType.BOLD}>
											{item.fineCost}
