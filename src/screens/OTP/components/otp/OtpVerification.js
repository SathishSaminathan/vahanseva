...
import { NavigationHeader, CustomScreenContainer, CustomText, CustomTextInput, CustomButton, FullButtonComponent } from '../../lib';
...
const OtpVerification = function (props) {
	...
	const sendOtp = () => {
		new Services()
			.api(GET, `vehicle/otp?otpType=VERIFY_OTP&vehicleNumber=${encodeURIComponent(VehicleId)}`)
			.then((res) => {})
			.catch((err) => {});
	};
	...
	const verifyOtp = () => {
		new Services()
			.api(GET, `vehicle/otp/verify?otp=${`${otpArray[0]}${otpArray[1]}${otpArray[2]}${otpArray[3]}`}`)
			.then((res) => {
		...
		<CustomScreenContainer>
			...
			<ErrorBoundary screenName={'OtpVerification'}>
				<View style={styles.container}>
					<CustomText>Enter OTP sent to register number {otpRequestData.email_id ? 'email' : 'mobile number'} </CustomText>
					...
					{errorMessage ? (
						<CustomText style={[GenericStyles.negativeText, GenericStyles.mt12, GenericStyles.centerAlignedText]}>
							{errorMessage}
						</CustomText>
					) : null}
            ...
            {attemptsRemaining || 0} Attempts remaining
					...
					{otpArray[0] !== '' && otpArray[1] !== '' && otpArray[2] !== '' && otpArray[3] !== '' && (
